...
import { Calendar, Plus, ExternalLink, RefreshCw, LogOut, Clock, Loader2 } from 'lucide-react';
...
export const GoogleCalendarTab: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    if (!auth || !googleProvider) {
      alert('Firebase Auth / Google Calendar is not yet configured for this applet.');
      return;
    }
    try {
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!eventTitle.trim() || !eventDate || !accessToken || creating) return;

    setCreating(true);
    try {
      const startDateTime = new Date(eventDate).toISOString();
      const endDateTime = new Date(new Date(eventDate).getTime() + 60 * 60 * 1000).toISOString();
      ...
      <div className="p-4 border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">View upcoming events, schedule meetings, and manage your calendar</p>
          </div>
        </div>
        ...
        {!user ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-16 space-y-6">
            ...
            <div className="max-w-md space-y-2">
              <h3 className="text-xl font-bold text-slate-900">Connect Google Calendar</h3>
              <p className="text-slate-600 text-sm">
                Sign in with your Google account to access and manage your calendar events securely.
              </p>
            </div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleCreateEvent} className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-wrap items-center gap-3 shadow-sm">
              <input
                type="text"
                value={eventTitle}
                onChange={(e) => setEventTitle(e.target.value)}
                placeholder="Event Title..."
                className="flex-1 min-w-[200px] bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500"
              />
              <input
                type="datetime-local"
                value={eventDate}
                onChange={(e) => setEventDate(e.target.value)}
                className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-indigo-500"
              />
              <button
                type="submit"
                disabled={creating || !eventTitle.trim() || !eventDate}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-indigo-600" /> Upcoming Calendar Events
                </h3>
                <button
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {events.map((ev) => (
                    <div key={ev.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between hover:border-slate-300 transition-colors">
                      <div className="flex items-start gap-3 min-w-0">
                        <div className="p-2.5 rounded-lg bg-indigo-50 text-indigo-600 shrink-0 border border-indigo-100 mt-0.5">
                          <Clock className="w-4 h-4" />
                        </div>
                        <div className="min-w-0">
                          <h4 className="font-semibold text-sm text-slate-800 truncate">{ev.summary}</h4>
                ...
                <div className="text-center py-12 text-slate-400 text-sm font-medium">
                  No upcoming calendar events found. Add your first event above!
                </div>
              )}
            </div>
          </div>
