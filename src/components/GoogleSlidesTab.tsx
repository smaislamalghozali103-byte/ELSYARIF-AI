...
import { Presentation, Plus, ExternalLink, RefreshCw, LogOut, Loader2, Sparkles } from 'lucide-react';
import { initializeApp, getApps } from 'firebase/app';
import { getAuth, signInWithPopup, GoogleAuthProvider, onAuthStateChanged, User } from 'firebase/auth';
import { GoogleSlideItem } from '../types';
...
export const GoogleSlidesTab: React.FC = () => {
  const [user, setUser] = useState<User | null>(null);
  const [accessToken, setAccessToken] = useState<string | null>(null);
  const [presentations, setPresentations] = useState<GoogleSlideItem[]>([]);
  ...
  const handleGoogleSignIn = async () => {
    if (!auth || !googleProvider) {
      alert('Firebase Auth / Google Slides is not yet configured for this applet.');
      return;
    }
    try {
      ...
      if (credential?.accessToken) {
        ...
        fetchPresentations(credential.accessToken);
      }
    } catch (err: any) {
  ...
  const fetchPresentations = async (token: string) => {
    setLoading(true);
    try {
      const res = await fetch("https://www.googleapis.com/drive/v3/files?q=mimeType='application/vnd.google-apps.presentation'", {
  ...
  const handleCreatePresentation = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim() || !accessToken || creating) return;
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50 text-slate-900 overflow-y-auto">
      <div className="p-4 border-b border-slate-200 bg-white/80 backdrop-blur-md flex items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-base text-slate-900">Google Slides Integration</h2>
            <p className="text-xs text-slate-500">Create and manage your Google Slides presentations seamlessly</p>
          </div>
        </div>
        ...
        {!user ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-16 space-y-6">
            ...
            <div className="max-w-md space-y-2">
              <h3 className="text-xl font-bold text-slate-900">Connect Google Slides</h3>
              <p className="text-slate-600 text-sm">
                Sign in with your Google account to access your presentations securely.
              </p>
            </div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleCreatePresentation} className="bg-white border border-slate-200 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
              <input
                type="text"
                value={newTitle}
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                  <Presentation className="w-4 h-4 text-indigo-600" /> Your Google Presentations
                </h3>
                <button
                  onClick={() => accessToken && fetchPresentations(accessToken)}
              ...
              {loading ? (
                <div className="flex items-center justify-center py-12 text-slate-500 gap-3">
                  ...
                  <span className="text-sm font-medium">Loading your Google Slides...</span>
                </div>
              ) : presentations.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {presentations.map((slide) => (
                    <div key={slide.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between hover:border-slate-300 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="p-2.5 rounded-lg bg-indigo-50 text-indigo-600 shrink-0 border border-indigo-100">
                          <Presentation className="w-5 h-5" />
                        </div>
                        <div className="min-w-0">
                          <h4 className="font-semibold text-sm text-slate-800 truncate">{slide.name || slide.title}</h4>
                ...
                <div className="text-center py-12 text-slate-400 text-sm font-medium">
                  No Google Slides presentations found in your account. Create your first presentation above!
                </div>
              )}
            </div>
          </div>
